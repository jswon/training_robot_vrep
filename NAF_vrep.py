  ...
  def __init__(self, namespace,
    ...
    with tf.variable_scope(namespace):
      ...
      for row_idx in range(action_dim):
        row_offset_in_l = int((row_idx*(row_idx+1))/2)
        ...
        rows.append(tf.concat([lower, diag, upper],1))
      ...
      P = tf.matmul(L, L_T)  # (batch_size, action_dim, action_dim)

      # can now calculate advantage
      u_mu_diff_T = tf.transpose(u_mu_diff, (0, 2, 1))
      advantage = -0.5 * tf.matmul(u_mu_diff_T, tf.matmul(P, u_mu_diff))  # (batch, 1, 1)
